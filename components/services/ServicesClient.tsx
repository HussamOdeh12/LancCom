      ...
      <main id="main-content" className="flex-1 text-start">
        ...
        <section className="py-10 bg-slate-950 border-b border-slate-800/80">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="p-6 sm:p-8 bg-slate-900/80 border border-slate-800 rounded-3xl grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
              ...
              <div className="md:col-span-4 flex justify-start md:justify-end">
                <Link
                  href="/contact"
                  className="px-6 py-3 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-slate-950 font-bold text-xs sm:text-sm rounded-xl transition-all shadow-lg shadow-cyan-500/20 flex items-center space-x-2 rtl:space-x-reverse"
                >
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {CORE_SERVICES.map((srv, index) => (
                <div
                  key={srv.id}
                  className="p-6 rounded-3xl bg-slate-900/90 border border-slate-800 hover:border-cyan-500/50 hover:bg-slate-900 transition-all duration-200 flex flex-col justify-between group shadow-lg"
                >
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="p-3 rounded-2xl bg-cyan-950 border border-cyan-500/30 text-cyan-400 group-hover:scale-105 transition-transform">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-cyan-400/90 font-medium mt-1 mb-2">
                    ...
                    <div className="pt-2 space-y-1.5 border-t border-slate-800/60">
                      {(language === 'ar' ? srv.featuresAr : srv.features).slice(0, 2).map((feat, idx) => (
                        <div key={idx} className="flex items-start space-x-1.5 rtl:space-x-reverse text-[11px] text-slate-300">
                  ...
                  <div className="pt-5 mt-4 border-t border-slate-800">
                    <Link
                      ...
                      className="w-full py-2.5 px-4 bg-slate-800 hover:bg-cyan-500 hover:text-slate-950 text-slate-200 font-bold text-xs rounded-xl transition-all flex items-center justify-between group/link"
                    >
        ...
        <section className="py-16 bg-slate-900 border-t border-b border-slate-800">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
              ...
              <Link
                href="/experience"
                className="inline-flex items-center space-x-1.5 rtl:space-x-reverse text-xs font-bold text-cyan-400 hover:text-cyan-300 transition-colors shrink-0"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {DOCUMENTED_EXPERIENCE.map((exp) => (
                <div
                  key={exp.id}
                  className="p-5 bg-slate-950 rounded-2xl border border-slate-800 space-y-3 text-start"
                >
                  <div className="flex items-center justify-between text-xs text-cyan-400">
                    <span className="font-semibold">{language === 'ar' ? exp.locationAr : exp.location}</span>
                    <Briefcase className="w-4 h-4 text-cyan-400/70" />
                  </div>
