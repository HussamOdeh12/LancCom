...
export function ExperienceClient() {
  ...
  const getServiceIcon = (slug: string) => {
    switch (slug) {
      case 'erp-systems': return <Database className="w-4 h-4 shrink-0 text-cyan-600 dark:text-cyan-400" />;
      case 'mobility-solutions': return <Smartphone className="w-4 h-4 shrink-0 text-cyan-600 dark:text-cyan-400" />;
      case 'it-resources-networking': return <Network className="w-4 h-4 shrink-0 text-cyan-600 dark:text-cyan-400" />;
      case 'security-solutions': return <ShieldCheck className="w-4 h-4 shrink-0 text-cyan-600 dark:text-cyan-400" />;
      case 'tracking-systems': return <Navigation className="w-4 h-4 shrink-0 text-cyan-600 dark:text-cyan-400" />;
      case 'rfid-solutions': return <Radio className="w-4 h-4 shrink-0 text-cyan-600 dark:text-cyan-400" />;
      case 'ecommerce-web': return <Globe className="w-4 h-4 shrink-0 text-cyan-600 dark:text-cyan-400" />;
      case 'managed-it-services': return <Headset className="w-4 h-4 shrink-0 text-cyan-600 dark:text-cyan-400" />;
      default: return <Layers className="w-4 h-4 shrink-0 text-cyan-600 dark:text-cyan-400" />;
    }
  };
      ...
      <main id="main-content" className="flex-1 text-start">
        ...
        <section className="py-10 bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800/80">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="p-6 sm:p-8 rounded-3xl bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 shadow-sm">
              <div className="flex items-start space-x-4 rtl:space-x-reverse">
                <div className="p-3 rounded-2xl bg-cyan-100 dark:bg-cyan-950 border border-cyan-200 dark:border-cyan-500/30 text-cyan-700 dark:text-cyan-400 shrink-0 mt-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {DOCUMENTED_EXPERIENCE.map((exp, idx) => (
                <div 
                  ...
                  className="bg-slate-50 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 hover:border-cyan-500/40 rounded-3xl p-6 sm:p-8 space-y-6 flex flex-col justify-between transition-all group shadow-sm hover:shadow-md"
                >
                  <div className="space-y-5">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-1.5 rtl:space-x-reverse text-xs text-emerald-600 dark:text-emerald-400 font-medium">
                    ...
                    {/* Organization Name */}
                    <div className="p-4 bg-white dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800 flex items-start space-x-3.5 rtl:space-x-reverse shadow-xs">
                      <div className="p-2.5 rounded-xl bg-cyan-100 dark:bg-cyan-950/60 border border-cyan-200 dark:border-cyan-500/20 text-cyan-700 dark:text-cyan-400 shrink-0 mt-0.5 group-hover:scale-105 transition-transform">
                    ...
                    {/* Scope of Work */}
                    <div className="p-4 bg-white dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800/80 space-y-2 shadow-xs">
                    ...
                    {exp.relatedServiceSlug && (
                      <div className="pt-2">
                        <Link
                          ...
                          className="p-3 bg-white dark:bg-slate-950/60 hover:bg-slate-50 dark:hover:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800/80 hover:border-cyan-500/40 flex items-center justify-between text-xs text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-all group/link shadow-xs"
                        >
                          <div className="flex items-center space-x-2 rtl:space-x-reverse min-w-0">
                            {getServiceIcon(exp.relatedServiceSlug)}
                            <span className="truncate font-semibold text-slate-800 dark:text-slate-200 group-hover/link:text-cyan-700 dark:group-hover/link:text-cyan-400 transition-colors">
                              {language === 'ar' ? exp.relatedServiceNameAr : exp.relatedServiceName}
                            </span>
                          </div>
                          <ChevronRight className={`w-4 h-4 text-slate-400 dark:text-slate-600 group-hover/link:text-cyan-700 dark:group-hover/link:text-cyan-400 shrink-0 transition-transform ${isRTL ? 'rotate-180 group-hover/link:-translate-x-0.5' : 'group-hover/link:translate-x-0.5'}`} />
                        </Link>
                      </div>
                    )}
                  </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {CORE_SERVICES.map((service) => (
                <Link
                  ...
                  className="p-5 rounded-2xl bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 hover:border-cyan-500/50 hover:bg-slate-50 dark:hover:bg-slate-950/80 transition-all space-y-3 block text-start group shadow-xs"
                >
                  <div className="flex items-center justify-between">
                    <div className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-cyan-700 dark:text-cyan-400 group-hover:scale-105 transition-transform">
                      {getServiceIcon(service.slug)}
                    </div>
                    <ChevronRight className={`w-4 h-4 text-slate-400 dark:text-slate-600 group-hover:text-cyan-700 dark:group-hover:text-cyan-400 shrink-0 transition-transform ${isRTL ? 'rotate-180 group-hover:-translate-x-0.5' : 'group-hover:translate-x-0.5'}`} />
                  </div>
                  <div>
                    <h3 className="text-sm font-bold text-slate-900 dark:text-white group-hover:text-cyan-700 dark:group-hover:text-cyan-400 transition-colors">
