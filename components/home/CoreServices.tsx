...
export function CoreServices() {
  ...
  return (
    <section id="services" className="py-20 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white relative text-start border-b border-slate-200 dark:border-slate-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            return (
              ...
              >
                <div>
                  ...
                  <h3 className="text-base font-bold text-slate-900 dark:text-white mb-1.5 group-hover:text-cyan-600 dark:group-hover:text-cyan-300 transition-colors">
                    {srv.title}
                  </h3>

                  <p className="text-xs text-cyan-700 dark:text-cyan-300/90 font-medium mb-3">
                    {srv.tagline}
                  </p>
                ...
                <div className="pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between text-xs">
                  <span className="text-[11px] font-semibold text-slate-600 dark:text-slate-300 group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors">
                    {language === 'ar' ? 'استعراض النطاق' : 'View Scope'}
                  </span>
                  <ArrowRight className={`w-3.5 h-3.5 text-slate-400 dark:text-slate-500 group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-transform ${isRTL ? 'rotate-180' : ''}`} />
                </div>
              </div>
            );
          })}
        ...
        <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 lg:p-10 shadow-xl dark:shadow-2xl relative overflow-hidden text-start">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            ...
            <div className="lg:col-span-8 space-y-5">
              <div className="flex items-center space-x-3 rtl:space-x-reverse">
                <div className="p-3 bg-cyan-100 dark:bg-cyan-500/10 border border-cyan-200 dark:border-cyan-500/30 rounded-xl text-cyan-700 dark:text-cyan-400 shrink-0">
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-900/60 rounded-xl border border-slate-200 dark:border-slate-800">
                <div className="text-xs font-bold text-cyan-700 dark:text-cyan-300 mb-1">
              ...
              <div className="space-y-2.5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  {selectedService.features.map((feat, idx) => (
                    <div key={idx} className="p-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800/80 flex items-start space-x-2.5 rtl:space-x-reverse shadow-xs">
                      <CheckCircle2 className="w-4 h-4 text-cyan-600 dark:text-cyan-400 shrink-0 mt-0.5" />
                      <span className="text-xs text-slate-700 dark:text-slate-200 font-medium">{feat}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {/* End: Action Box */}
            <div className="lg:col-span-4 bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 space-y-5 text-start shadow-xs">
              <div className="space-y-2">
                <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded bg-cyan-100 dark:bg-cyan-950 text-cyan-800 dark:text-cyan-300 border border-cyan-200 dark:border-cyan-800/40">
