...
export function LanguageSwitcher({ variant = 'navbar', className = '' }: LanguageSwitcherProps) {
    ...
    return (
      <div className={`p-3 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 ${className}`}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center p-1 rounded-lg bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800" role="group" aria-label="Language selection">
            <button
              type="button"
  ...
  return (
    <div className={`relative inline-flex items-center ${className}`}>
      ...
      >
        <Globe className="w-3.5 h-3.5 text-cyan-600 dark:text-cyan-400 group-hover:rotate-45 transition-transform duration-300" />
